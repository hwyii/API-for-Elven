...
def processDataDeposit(apiKey, SecretKey, passphrase, beginTime=None, endTime=None, timezone=None):
...
# https://www.okx.com/docs-v5/zh/?python#funding-account-rest-api-get-deposit-history
def processDataWithdraw(apiKey, SecretKey, passphrase, beginTime=None, endTime=None, timezone=None):
    function_startTime = time.time()
    # endpoint
    base_url = 'https://aws.okx.com'
...
## 获取成交明细（近三个月）trade
# https://www.okx.com/docs-v5/zh/?python#order-book-trading-trade-get-transaction-details-last-3-months
def processDataTrade(apiKey, SecretKey, beginTime=None, endTime=None, timezone=None):
    function_startTime = time.time()
    # 创建一个空的 DataFrame，用于存放结果
    all_transfers = pd.DataFrame()
    ...
    while True:
        ...
        # 请求路径
        endpoint_trade ="/api/v5/trade/fills-history?instType=SWAP&begin=" + str(startTimestamp) + "&end=" + str(endTimestamp)
        ...
        try:
            ...
            if df.empty:
                ...
                while trade_num == 100: # 数据条数超出limit限制
                    ...
                    endpoint_trade_in ="/api/v5/trade/fills-history?instType=SWAP&begin=" + str(startTimestamp) + "&end=" + str(endTimestamp) + "&after=" + str(min_id)
                    body_trade_in = ""
